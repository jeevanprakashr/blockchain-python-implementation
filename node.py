'''
Examples to start the node: python node.py
...
def add_node():
    ...
    if blockchain.add_peer_node(node):
        response = {
        ...
        response = {"message": "Node doesn't exist! Please check the url."}
        return jsonify(response), 400
...
def get_key():
    if wallet.public_key:
        ...
        return jsonify(response), 200
    response = {"message": "Node doesn't have a wallet."}
    return jsonify(response), 500

@app.route('/peerkey', methods=['POST'])
def get_peer_key():
    values = request.get_json()
    if not values:
        response = {'message': 'No data found.'}
    ...
    if 'node' not in values:
        response = {'message': 'Some data is missing.'}
        return jsonify(response), 400
    result = Blockchain.get_peer_key(values['node'])
    if result:
